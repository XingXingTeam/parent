...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CourseTableViewCell *cell =(CourseTableViewCell*)[tableView dequeueReusableCellWithIdentifier:COURSE];
    if (cell == nil) {
        NSArray *nib =[[NSBundle mainBundle] loadNibNamed:COURSE owner:[CourseTableViewCell class] options:nil];
        cell =(CourseTableViewCell *)[nib objectAtIndex:0];
    }
...
//
    ...
    if ([_teacherNameArray[indexPath.row] count] != 0) {
        ...
        for (int i = 0; i < [_teacherNameArray[indexPath.row] count]; i++) {
            UILabel *teacherNameLabel = [HHControl createLabelWithFrame:CGRectMake(155 + 100 * i, 30, 80, 20) Font:14 Text:_teacherNameArray[indexPath.row][i]];
            [cell.contentView addSubview:teacherNameLabel];
        }
        
    }

    cell.totalNumberLabel.text = _totalNumberArray[indexPath.row];
    cell.numberLabel.text = self.numberArray[indexPath.row];
    cell.oldPriceLabel.text = self.oldPriceArray[indexPath.row];
    cell.nowPriceLbl.text = self.nowPriceArray[indexPath.row];
