...
- (void)createContent{
   //输入新 手机号
    ...
    submitButton = [HHControl createButtonWithFrame:CGRectMake(buttonX, buttonY, buttonW, buttonH) backGruondImageName:@"login_green" Target:self Action:@selector(submitButton:) Title:@"提       交"];
...
- (void)checkPhoneNumberWithPhone:(NSString *)phone
{
    ...
    /*
     ...
     接口类型:1
     接口:
     http://www.xingxingedu.cn/Teacher/register_check_phone
     传参(参数名):
     phone 		//手机
     */
    NSString *urlStr = @"http://www.xingxingedu.cn/Teacher/register_check_phone";
    NSDictionary *params = @{@"appkey":APPKEY,
                             @"backtype":BACKTYPE,
                             @"xid":parameterXid,
                             @"user_id":parameterUser_Id,
