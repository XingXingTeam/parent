...
- (void)getXingCoinInfo{
    ...
    [WZYHttpTool post:urlStr params:params success:^(id responseObj) {
        //
//        NSLog(@"bbb %@", responseObj);
        
        if ([responseObj[@"code"] integerValue] == 1) {
            
            NSString *coinAble = responseObj[@"data"][@"coin_able"];
            
            moneyRemainLabel.text= coinAble;
            moneyText.placeholder=[NSString stringWithFormat:@"最多可转%@猩币",coinAble];
            self.moneyRemain=coinAble;
        }
...
//        NSLog(@"%@", xidDict[self.familyCombox.textField.text]);
