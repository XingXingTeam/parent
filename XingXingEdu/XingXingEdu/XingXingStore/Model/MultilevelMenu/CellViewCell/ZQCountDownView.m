...
- (void)adjustSubViewsWithFrame:(CGRect)frame
{
    ...
    _hourLabel.minimumScaleFactor=0.1;
    _secondLabel.adjustsFontSizeToFitWidth=YES;
    _secondLabel.minimumScaleFactor=0.1;

    _minuteLabel.adjustsFontSizeToFitWidth=YES;
    _minuteLabel.minimumScaleFactor=0.1;
...
- (void)setCountDownTimeInterval:(NSTimeInterval)countDownTimeInterval
{
    ...
    _hour = _countDownTimeInterval / 3600;
     [self setHourText:[NSString stringWithFormat:@"%02d时", _hour]];
    _minuteLabel.text = [NSString stringWithFormat:@"%02d分", _minute];
    _secondLabel.text = [NSString stringWithFormat:@"%02d秒", _second];
