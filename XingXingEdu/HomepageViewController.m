...
//    tit.text = [NSString stringWithFormat:@"签名:%@",personl_signArray[0]];
    ...
    for (int a =0; a<(image.count+1); a++) {
        ...
        if (a==image.count) {
            ...
            UserImage.tag = 100 + a;
            //性别符号
            manimage = [[UIImageView alloc] initWithFrame:CGRectMake(50 * kWidth / 375 , 90 * kWidth / 375, 20 * kWidth / 375 , 20 * kWidth / 375)];
...
//    self.schoolNameCombox.textField.placeholder = SCHOOL_NAME;
    ...
    if ([DEFAULTS objectForKey:@"RCUserToken"] == nil) {
        ...
        //班级选择
        self.gradeAndClassbox = [[WJCommboxView alloc]initWithFrame:CGRectMake(self.schoolNameCombox.frame.origin.x + self.schoolNameCombox.frame.size.width + 5,  37 * kWidth /375 , 120 * kWidth / 375, 30* kHeight / 667)];
    }else{
    
        //班级选择
        self.gradeAndClassbox = [[WJCommboxView alloc]initWithFrame:CGRectMake(self.schoolNameCombox.frame.origin.x + self.schoolNameCombox.frame.size.width + 5,  37 * kWidth /375 , 150 * kWidth / 375, 30* kHeight / 667)];
    }
...
- (void)createSchoolInfo:(NSInteger) index{
    ...
    NSLog(@"班级 %@", commboxGradeAndClassArray);
}
...
- (void)createMiddleBackgroundContent{
     ...
     *  小红花
     */
      flowerBtn = [HHControl createButtonWithFrame:CGRectMake((191 + 30) * kWidth / 375, 5, 31 * kWidth / 375, 31 * kWidth / 375) backGruondImageName:@"xiaohonghua45x45" Target:self Action:@selector(onClickflower:) Title:nil];
        [middleBackgroundView addSubview:flowerBtn];
    /**
     *  花篮
     */
        flowersBtn = [HHControl createButtonWithFrame:CGRectMake(5 * kWidth / 375, 5, 32 * kWidth / 375, 32 * kWidth / 375) backGruondImageName:@"花篮52x56" Target:self Action:@selector(oncliCkFlowers:) Title:nil];
        [middleBackgroundView addSubview:flowersBtn];
    /**
     *  猩币
     */
        xingbiBtn = [HHControl createButtonWithFrame:CGRectMake(96 * kWidth / 375, 5, 35 * kWidth / 375, 31 * kWidth / 375) backGruondImageName:@"猩币52x46" Target:self Action:@selector(onClickxingbi:) Title:nil];
        [middleBackgroundView addSubview:xingbiBtn];
    /**
     *  通知
     */
        hurnBtn = [HHControl createButtonWithFrame:CGRectMake(313 * kWidth / 375, 5, 25 * kWidth / 375, 32 * kWidth / 375) backGruondImageName:@"通知38x50" Target:self Action:@selector(onClickhurn:) Title:nil];
...
-(void)createLabels
{
   ...
   levelLabel = [HHControl createLabelWithFrame:CGRectMake(nameLabel.frame.origin.x + nameLabel.frame.size.width + 10 ,nameLabel.centerY - 20 * kHeight / 667 / 2 , 35 * kWidth / 375, 20 * kHeight / 667) Font:12 * kWidth / 375 Text:lvString];
    ...
    //宝贝 年龄
    ageLabel =[HHControl createLabelWithFrame:CGRectMake(kWidth / 2 , nameLabel.frame.origin.y + nameLabel.frame.size.height + 5, 160 * kWidth / 375, 30 * kHeight / 667) Font:16 * kWidth / 375 Text:@""];
    ageLabel.textColor =UIColorFromRGB(255, 255, 255);
    [upBackgroundImageView addSubview:ageLabel];
    
    //签名
    titleLabel =[HHControl createLabelWithFrame:CGRectMake(kWidth / 2 , ageLabel.frame.origin.y + ageLabel.frame.size.height + 5, 160 * kWidth / 375, 40 * kHeight / 667) Font:16 * kWidth / 375 Text:@""];
    ...
    //花数
    fnumberLabel = [HHControl createLabelWithFrame:CGRectMake((228 + 30)* kWidth / 375, 5 * kWidth / 375, 50 * kWidth / 375, 30 *kWidth / 375) Font:16 * kWidth / 375 Text:@""];
    [middleBackgroundView addSubview:fnumberLabel];
    
    //花篮数
    fsnumberLabel = [HHControl createLabelWithFrame:CGRectMake(43 * kWidth / 375, 5 * kWidth / 375, 45 * kWidth / 375, 30 * kWidth / 375) Font:16 * kWidth / 375 Text:@""];
    [middleBackgroundView addSubview:fsnumberLabel];
    
    //星币
    xnumberLabel = [HHControl createLabelWithFrame:CGRectMake(137 * kWidth / 375, 5 * kWidth / 375, 80 * kWidth / 375, 30 * kWidth / 375) Font:16 * kWidth / 375 Text:@""];
