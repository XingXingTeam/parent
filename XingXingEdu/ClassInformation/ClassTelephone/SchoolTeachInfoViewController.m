//
//  SchoolTeachInfoViewController.m
//  XingXingEdu
...
#import "HHControl.h"
@interface SchoolTeachInfoViewController ()<QHNavSliderMenuDelegate,UIScrollViewDelegate>
{
...
@implementation SchoolTeachInfoViewController
...
- (void)initView{
    ...
    navSliderMenu = [[QHNavSliderMenu alloc] initWithFrame:(CGRect){0,294,kWidth+100,30} andStyleModel:model andDelegate:self showType:self.menuType];
...
- (void)createPeopleInfo{
    ...
    UIButton *nametitleBtn =[HHControl createButtonWithFrame:CGRectMake(50,236, 170, 20) backGruondImageName:@"" Target:self Action:@selector(nameBtn) Title:@"注册教师: 20"];
    [self.view addSubview:nametitleBtn];
    
    
    UILabel *nicknameLabel = [[UILabel alloc] initWithFrame:CGRectMake(nametitleBtn.frame.origin.x+170, 236, 200, 20)];
    NSString *nickname = @" 注册学生: 1200";
    nicknameLabel.text = nickname;
    nicknameLabel.font = [UIFont systemFontOfSize:15];
    [self.view addSubview:nicknameLabel];
    //  星级评分
    UIButton *starBtn =[HHControl createButtonWithFrame:CGRectMake(30, 266, 90, 20) backGruondImageName:@"" Target:self Action:@selector(staBtn) Title:[NSString stringWithFormat:@"星级评分: %@",@"20"]];
    starBtn.titleLabel.font =[UIFont systemFontOfSize:15];
    [self.view addSubview:starBtn];
    
    
    
    UILabel *teletitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(150,266, kWidth-100, 20)];
    ...
    UILabel *scoreLabel =[[UILabel alloc]initWithFrame:CGRectMake(150, 211, 200, 20)];
