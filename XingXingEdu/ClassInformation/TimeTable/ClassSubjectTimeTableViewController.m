...
//    NSInteger page = scrollView.contentOffset.y / kHeight;
//    if (_number > 0) {
//        page = _number -1 ; //
//        
//    }
...
//    }
    ...
    titleCombox.textField.text = classNameArr[page];
    
//        if (_number == 0) {
//             titleCombox.textField.text = classNameArr[page];
//        }
    
//        NSString *wekStr =[weekMArr objectAtIndex: page ];
//            /**
//             *切换时间表 传weekStr
//             */
//        [self configUI:wekStr];
...
- (void)landClick:(UIBarButtonItem*)editBtn{
    ...
    classSubjectVC.weekNumStr = [NSString stringWithFormat:@"%ld",_number];
...
- (void)configUI{
    ...
    for (int kti=0 ; kti<=classNameArr.count; kti++) {
        UILabel *monthLbl =[HHControl createLabelWithFrame:CGRectMake(1, 0+itemStartY, 26, 40) Font:10 Text:@"7月"];
    ...
    for (NSInteger ikt =1; ikt<= tmMArr.count; ikt++) {
        ...
        for (NSInteger jkt =1; jkt<=7; jkt++) {
            ...
            if ([[_dataDict objectForKey:[NSString stringWithFormat:@"%ld",(long)classBtn.tag]] rangeOfString:@"&"].location !=NSNotFound) {
                ...
                UIButton *redBtn = [[UIButton alloc]initWithFrame:CGRectMake(25,0, 15, 15) ];
...
- (void)showEditPopover:(UIButton*)btn{
    ...
    subjectInfoVC.className = [NSString stringWithFormat:@"%@",[_dataDict objectForKey:[NSString stringWithFormat:@"%ld",btn.tag]]];
